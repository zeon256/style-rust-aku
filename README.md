...
| **`no_inline_modules`** | Warns against inline test module bodies: `#[cfg(test)] mod tests { ... }` (including compound `all`/`any` cfg predicates) and ungated inline modules whose body directly registers a test function. Suggests replacing the `{ ... }` body with `;`.<br><br>**Bad:** `#[cfg(test)] mod tests { fn helper() {} }`<br>**Good:** `#[cfg(test)] mod tests;` | Keeps test module bodies in standalone module files that `rustc` resolves, so cfg attributes and visibility are preserved and the body is testable in isolation. | Yes |
| **`no_tests_outside_test_files`** | Warns against functions carrying test-registering attributes (`test`, `rstest`, `test_case`, `parameterized`, `test_with`, `wasm_bindgen_test`, `quickcheck`, `proptest`, `test_matrix`, `test_each_path`, and scoped forms like `tokio::test`) defined outside `tests.rs`, `*_tests.rs`, or `tests/**/*.rs`. Companion-only attributes (`serial`, `fixture`, `should_panic`) do not fire. | Keeps all tests in `rustc`-resolved standalone test files so `cargo test` discovers them uniformly. | No |
...
- `no_inline_modules` and `no_tests_outside_test_files` run **pre-expansion**, so they inspect raw source before cfg-stripping and macro expansion: gated modules are flagged even in non-test builds, and attribute-macro tests (`#[tokio::test]`, `#[rstest]`, ...) are matched on the attribute as written, not on its expansion. `no_inline_modules` only flags inline module bodies (`mod foo { ... }`); file-backed `mod foo;` declarations are always allowed.
